...
bulk_peaks_24h = last_file_generated(
  parent_folder = "/media/rparmentier/Elements/DATA_MARS-ATAC/ATACseq",
  pattern = "peaks_intersection_24h_MP_ann.csv")
    
# General list (union) of peaks obtained in bulk ATAC
bulk_peaks_24h <- read_delim(
  "/media/rparmentier/Elements/DATA_MARS-ATAC/ATACseq/peaks_intersection_24h_MP_ann.csv",
...
sc_peaks_24h_df = read_table(
  file = "~/Bureau/sc_ATAC_analysis/data/peaks_hg19.bed", 
  col_names = FALSE)
...
all_annotations = loadRData(
  "~/Bureau/sc_ATAC_analysis/data/Annotation_TSS_pm1kb_int_ex_53utr_ctcf_cpg_woThisto_FANTOM5_prom_gr.rda")
...
sc_sharing_stats = last_file_generated(parent_folder = "~/Bureau/sc_ATAC_analysis/exp/cells_peaks_statistics/", pattern = ".csv" )

# Load single-cell sharing statistics data.frame
sc_sharing_stats = read.delim(sc_sharing_stats, sep = ";")
# sc_sharing_stats = loadRData("~/Bureau/sc_ATAC_analysis/exp/cells_peaks_statistics/2022_01_19_12h_33min/sc_sharing_stats_df.rda")

sc_sharing_stats_df = tibble(
  peak_name = sc_sharing_stats$peak_name,
  nb_cells_sharing_this_peak = sc_sharing_stats$sc_sharing_stats_df)
...
all_peaks_shared_plot = ggplot(sc_sharing_stat_overlapped, aes(x = nb_cells_sharing_this_peak, color = overlap_bulk_at_24h)) +
  ...
  geom_vline(aes(xintercept=sharing_cell_med, color=overlap_bulk_at_24h),
             linetype="dashed") +
  ggtitle(label = "Nb cells sharing peaks distribution", subtitle = "All peaks (253 591), all annotation")
...
sc_sharing_stat_overlapped_sampled = sc_sharing_stat_overlapped %>% 
  ...
  mutate(sharing_cell_mean = mean(nb_cells_sharing_this_peak))
...
sampled_peaks_shared_plot = ggplot(sc_sharing_stat_overlapped_sampled, aes(x = nb_cells_sharing_this_peak, color = overlap_bulk_at_24h)) +
  ...
  geom_vline(aes(xintercept=sharing_cell_mean, color=overlap_bulk_at_24h),
             linetype="dashed") +
    ggtitle(label = "Nb cells sharing peaks distribution", subtitle = "Non-overlapping peaks were sampled to match the number \n of overlapping peaks (n = 42345 ), all annotation")
...
sc_overlapped_prom_peaks = sc_overlapped_prom_peaks %>% 
  mutate(overlap_bulk_at_24h = replace(overlap_bulk_at_24h, is.na(overlap_bulk_at_24h), "FALSE")) %>%
  group_by(overlap_bulk_at_24h) %>%
  mutate(sharing_cell_mean = median(nb_cells_sharing_this_peak))
...
peak_cell_count_matrix = last_file_generated(parent_folder = "~/Bureau/sc_ATAC_analysis/exp/cells_peaks_statistics/", pattern = "peak_cell_count_matrix")
